...
class Safety(Node):
    ...
    def __init__(self):
        ...
        self.obstacle_info.distance = float('inf')
        self.obstacle_info.direction = ""
        self.threshold = self.obstacle_info.threshold = 1.0   # default threshold
    ...
    def safety_controller(self):
        ...
        if self.safe and not self.pub_once:
        ...
        if self.obstacle_info.distance <= self.threshold:
            ...
            print("Threshold passed! Saving the robot\n")
    ...
    def get_averages_callback(self, request, response):
        ...
        print(f"Linear average: {response.linear_avg}\nangular average: {response.angular_avg}")
        return response
